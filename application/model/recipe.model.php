...
class RecipeModel extends AbstractModel
{
	...
	if ( count($selectedTags) > 0) {
		...
		foreach( $selectedTags as $k => $tag ) {
			...
			if ( $k == 0 ) { $select = "SELECT sub0.entry_id as entry_id FROM entries_tags as sub{$k}"; $where = "tag_id = '{$tag_id}'"; }
			if ( $k  > 0 ) { $sqls[] = "INNER JOIN ( SELECT DISTINCT entry_id FROM entries_tags WHERE tag_id = '{$tag_id}' ) sub{$k} ON ( sub0.entry_id = sub{$k}.entry_id )"; }
		}
