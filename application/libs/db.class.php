...
final class DB
{
	...
	public function connect( $cfg ) 
	{
		...
		$user = $cfg['user'];
		$pass = $cfg['pass'];
		$char = "utf8"; if (isset($cfg['char'])) { $char = $cfg['char']; }
	...
	{
		$result = "";
		if( $this->query == "SELECT" ) { $result = $this->buildSelect();  }
		if( $this->query == "INSERT" ) { $result = $this->buildInsert();  }
		if( $this->query == "UPDATE" ) { $result = $this->buildUpdate();  }
		if( $this->query == "REPLACE") { $result = $this->buildReplace(); }
		if( $this->query == "DELETE" ) { $result = $this->buildDelete();  }
		if( $this->query == "COUNT"  ) { $result = $this->buildCount() ;  }
		if ( trim( $result ) !== ""  ) { $this->sqlQuery = trim( $result ); }
	}
      	/**
	...
	{
		Util::Dump( $this->sqlQuery ); if ( $stop ) { die(); } return $this;
	}
      	/**
